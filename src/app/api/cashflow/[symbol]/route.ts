import { NextRequest, NextResponse } from 'next/server';
import fs from 'fs';
import path from 'path';
import { ApiResponseParser } from '@/lib/api-parser';

const RAPIDAPI_KEY = process.env.RAPIDAPI_KEY;
const RAPIDAPI_HOST = 'insightsentry.p.rapidapi.com';

// Generate mock cash flow data for fallback
function generateMockCashFlowData(symbol: string) {
  const isBank = symbol.includes('BANK') || symbol.includes('HDFC') || symbol.includes('ICICI') || symbol.includes('SBIN');
  const years = 12;
  const historicalYears = Array.from({ length: years }, (_, i) => (2014 + i).toString()).reverse();
  
  // Generate realistic historical progression
  const generateHistoricalData = (base: number, growthRate: number = 0.10) => {
    return Array.from({ length: years }, (_, i) => {
      const factor = Math.pow(1 + growthRate, i - (years - 1));
      return Math.round(base * factor);
    }).reverse();
  };
  
  const baseData = {
    years: historicalYears,
    company_name: symbol,
    sector: isBank ? 'Private Sector Bank' : 'Technology',
    'sector-i18n-en': isBank ? 'Private Sector Bank' : 'Technology',
    report_type: isBank ? 'banking' : 'non-banking'
  };
  
  if (isBank) {
    // Banking-specific cash flow mock data
    return {
      ...baseData,
      // Operating Activities
      cash_f_operating_activities_fy_h: generateHistoricalData(143500, 0.15),
      funds_f_operations_fy_h: generateHistoricalData(97900, 0.12),
      changes_in_working_capital_fy_h: generateHistoricalData(45500, 0.18),
      increase_in_deposits_fy_h: generateHistoricalData(25000, 0.11),
      increase_in_loans_fy_h: generateHistoricalData(-18000, 0.13),
      
      // Investing Activities
      cash_f_investing_activities_fy_h: generateHistoricalData(-351700, 0.20),
      capital_expenditures_fy_h: generateHistoricalData(-4100, 0.08),
      purchase_sale_investments_fy_h: generateHistoricalData(-177900, 0.22),
      
      // Financing Activities
      cash_f_financing_activities_fy_h: generateHistoricalData(229100, 0.25),
      total_cash_dividends_paid_fy_h: generateHistoricalData(-14800, 0.10),
      issuance_of_stock_net_fy_h: generateHistoricalData(6300, 0.15),
      issuance_of_debt_net_fy_h: generateHistoricalData(-96400, 0.18)
    };
  } else {
    // Non-banking cash flow mock data
    return {
      ...baseData,
      // Operating Activities
      cash_f_operating_activities_fy_h: generateHistoricalData(25000, 0.12),
      net_income_fy_h: generateHistoricalData(18000, 0.15),
      cash_flow_deprecation_n_amortization_fy_h: generateHistoricalData(3800, 0.08),
      changes_in_working_capital_fy_h: generateHistoricalData(2500, 0.20),
      
      // Investing Activities
      cash_f_investing_activities_fy_h: generateHistoricalData(-8500, 0.15),
      capital_expenditures_fy_h: generateHistoricalData(-4100, 0.08),
      purchase_of_investments_fy_h: generateHistoricalData(-3200, 0.12),
      sales_of_investments_fy_h: generateHistoricalData(2400, 0.10),
      
      // Financing Activities
      cash_f_financing_activities_fy_h: generateHistoricalData(-12000, 0.18),
      total_cash_dividends_paid_fy_h: generateHistoricalData(-8500, 0.10),
      issuance_of_stock_net_fy_h: generateHistoricalData(1500, 0.12),
      purchase_of_stock_fy_h: generateHistoricalData(-2000, 0.08)
    };
  }
}

// Map real API response data to cash flow format with historical arrays
function mapApiToCashFlowData(apiData: any, symbol: string, isFromLocalFile: boolean = false) {
  console.log(`Mapping API data for ${symbol}, fromLocalFile: ${isFromLocalFile}`);
  
  const isBank = symbol.includes('BANK') || symbol.includes('HDFC') || symbol.includes('ICICI') || symbol.includes('SBIN');
  
  // Create a field map depending on the data source
  const fieldMap = new Map();
  
  if (isFromLocalFile && Array.isArray(apiData)) {
    // Local JSON file format: array of {id, value} objects
    apiData.forEach((field: any) => {
      if (field.id && field.value !== undefined) {
        fieldMap.set(field.id, field.value);
      }
    });
    console.log(`Loaded ${fieldMap.size} fields from local JSON`);
  } else if (apiData && typeof apiData === 'object') {
    // V3 API format: flat object with field names as keys
    Object.entries(apiData).forEach(([key, value]) => {
      fieldMap.set(key, value);
    });
    console.log(`Loaded ${fieldMap.size} fields from API response`);
  }
  
  console.log('Sample fields:', Array.from(fieldMap.keys()).slice(0, 10));
  console.log('Cash flow fields found:', Array.from(fieldMap.keys()).filter(key => 
    key.includes('cash_f_') || key.includes('_cash_flow') || key.includes('dividends') || key.includes('capital_expenditures')
  ).slice(0, 10));
  
  // Generate historical data arrays from current values (for testing)
  function generateHistoricalArray(currentValue: number, years: number = 12, growthRate: number = 0.15): number[] {
    if (!currentValue || isNaN(currentValue)) return new Array(years).fill(0);
    
    return Array.from({ length: years }, (_, i) => {
      // Generate backward progression
      const yearIndex = years - 1 - i;
      const adjustedValue = currentValue / Math.pow(1 + growthRate, yearIndex);
      return Math.round(adjustedValue);
    }).reverse();
  }
  
  const years = 12;
  const historicalYears: string[] = Array.from({ length: years }, (_, i) => (2014 + i).toString()).reverse();
  
  const cashFlowData: any = {
    years: historicalYears,
    company_name: symbol,
    sector: fieldMap.get('sector-i18n-en') || fieldMap.get('sector') || (isBank ? 'Private Sector Bank' : 'Technology'),
    'sector-i18n-en': fieldMap.get('sector-i18n-en') || fieldMap.get('sector'),
    report_type: isBank ? 'banking' : 'non-banking'
  };
  
  // Map cash flow fields based on API response - using actual field names from API
  const fieldMappings = [
    // Free Cash Flow
    { apiField: 'free_cash_flow_fy', histField: 'free_cash_flow_fy_h', growth: 0.15 },
    // Operating Activities
    { apiField: 'cash_f_operating_activities_fy', histField: 'cash_f_operating_activities_fy_h', growth: 0.15 },
    { apiField: 'net_income_fy', histField: 'net_income_fy_h', growth: 0.12 },
    { apiField: 'cash_flow_deprecation_n_amortization_fy', histField: 'cash_flow_deprecation_n_amortization_fy_h', growth: 0.08 },
    { apiField: 'funds_f_operations_fy', histField: 'funds_f_operations_fy_h', growth: 0.12 },
    { apiField: 'changes_in_working_capital_fy', histField: 'changes_in_working_capital_fy_h', growth: 0.18 },
    
    // Investing Activities
    { apiField: 'cash_f_investing_activities_fy', histField: 'cash_f_investing_activities_fy_h', growth: 0.20 },
    { apiField: 'capital_expenditures_fy', histField: 'capital_expenditures_fy_h', growth: 0.08 },
    { apiField: 'capital_expenditures_fixed_assets_fy', histField: 'capital_expenditures_fixed_assets_fy_h', growth: 0.08 },
    { apiField: 'purchase_sale_investments_fy', histField: 'purchase_sale_investments_fy_h', growth: 0.22 },
    { apiField: 'purchase_of_investments_fy', histField: 'purchase_of_investments_fy_h', growth: 0.15 },
    { apiField: 'sales_of_investments_fy', histField: 'sales_of_investments_fy_h', growth: 0.12 },
    { apiField: 'purchase_of_business_fy', histField: 'purchase_of_business_fy_h', growth: 0.10 },
    { apiField: 'sales_of_business_fy', histField: 'sales_of_business_fy_h', growth: 0.10 },
    
    // Financing Activities
    { apiField: 'cash_f_financing_activities_fy', histField: 'cash_f_financing_activities_fy_h', growth: 0.25 },
    { apiField: 'total_cash_dividends_paid_fy', histField: 'total_cash_dividends_paid_fy_h', growth: 0.10 },
    { apiField: 'common_dividends_cash_flow_fy', histField: 'common_dividends_cash_flow_fy_h', growth: 0.10 },
    { apiField: 'issuance_of_stock_net_fy', histField: 'issuance_of_stock_net_fy_h', growth: 0.15 },
    { apiField: 'sale_of_stock_fy', histField: 'sale_of_stock_fy_h', growth: 0.15 },
    { apiField: 'purchase_of_stock_fy', histField: 'purchase_of_stock_fy_h', growth: 0.08 },
    { apiField: 'issuance_of_debt_net_fy', histField: 'issuance_of_debt_net_fy_h', growth: 0.18 },
    { apiField: 'issuance_of_long_term_debt_fy', histField: 'issuance_of_long_term_debt_fy_h', growth: 0.15 },
    { apiField: 'reduction_of_long_term_debt_fy', histField: 'reduction_of_long_term_debt_fy_h', growth: 0.15 }
  ];
  
  // Add banking-specific fields if it's a bank
  if (isBank) {
    fieldMappings.push(
      { apiField: 'increase_in_deposits_fy', histField: 'increase_in_deposits_fy_h', growth: 0.11 },
      { apiField: 'increase_in_loans_fy', histField: 'increase_in_loans_fy_h', growth: 0.13 }
    );
  }
  
  // Map each field - prefer historical data if available
  fieldMappings.forEach(({ apiField, histField, growth }) => {
    // First, check if historical data already exists
    const historicalData = fieldMap.get(histField);
    if (historicalData && Array.isArray(historicalData) && historicalData.length > 0) {
      // Filter out null values and use the data
      const cleanData = historicalData.filter((v: any) => v !== null && v !== undefined);
      if (cleanData.length > 0) {
        cashFlowData[histField] = historicalData;
        console.log(`Using existing historical data for ${histField} (${historicalData.length} values)`);
      } else {
        // All values are null, use current value fallback
        const currentValue = fieldMap.get(apiField);
        if (currentValue && typeof currentValue === 'number') {
          cashFlowData[histField] = generateHistoricalArray(currentValue, years, growth);
          console.log(`Generated from current value for ${histField}`);
        } else {
          cashFlowData[histField] = new Array(years).fill(0);
          console.log(`No valid data for ${histField}, using zeros`);
        }
      }
    } else {
      // Fallback to generating from current value
      const currentValue = fieldMap.get(apiField);
      if (currentValue && typeof currentValue === 'number') {
        cashFlowData[histField] = generateHistoricalArray(currentValue, years, growth);
        console.log(`Generated historical data for ${apiField} (${currentValue}) -> ${histField}`);
      } else {
        // Use zeros if no data available
        cashFlowData[histField] = new Array(years).fill(0);
        console.log(`No data for ${apiField}/${histField}, using zeros`);
      }
    }
  });
  
  // Ensure sector information is properly set
  if (!cashFlowData.sector) {
    cashFlowData.sector = isBank ? 'Private Sector Bank' : 'Technology';
  }
  cashFlowData['sector-i18n-en'] = cashFlowData['sector-i18n-en'] || cashFlowData.sector;
  
  console.log('Generated cash flow data:', {
    symbol,
    isBank,
    sector: cashFlowData.sector,
    reportType: cashFlowData.report_type,
    fieldsGenerated: Object.keys(cashFlowData).filter(k => k.endsWith('_fy_h')),
    yearsCount: cashFlowData.years.length
  });
  
  return cashFlowData;
}

// Fetch real API data from RapidAPI InsightSentry
async function fetchRealApiData(symbol: string): Promise<any> {
  if (!RAPIDAPI_KEY) {
    console.error('RAPIDAPI_KEY is not configured');
    throw new Error('API key not configured');
  }

  try {
    // Use the full symbol with exchange prefix for the v3 API
    const fullSymbol = symbol.includes(':') ? symbol : `NSE:${symbol}`;
    
    console.log(`Fetching cash flow data from RapidAPI for symbol: ${fullSymbol}`);
    
    // Define the cash flow fields we need based on the API response and mapping
    const cashFlowFields = [
      // Free Cash Flow
      'free_cash_flow_fy_h',
      'free_cash_flow_fy',
      // Operating Activities - Historical
      'cash_f_operating_activities_fy_h',
      'net_income_fy_h', 
      'cash_flow_deprecation_n_amortization_fy_h',
      'funds_f_operations_fy_h',
      'changes_in_working_capital_fy_h',
      'increase_in_deposits_fy_h', // Banking
      'increase_in_loans_fy_h', // Banking
      
      // Investing Activities - Historical  
      'cash_f_investing_activities_fy_h',
      'capital_expenditures_fy_h',
      'capital_expenditures_fixed_assets_fy_h',
      'purchase_sale_investments_fy_h',
      'purchase_of_investments_fy_h',
      'sales_of_investments_fy_h',
      'purchase_of_business_fy_h',
      'sales_of_business_fy_h',
      'other_investing_cash_flow_items_total_fy_h',
      
      // Financing Activities - Historical
      'cash_f_financing_activities_fy_h',
      'total_cash_dividends_paid_fy_h',
      'common_dividends_cash_flow_fy_h',
      'issuance_of_stock_net_fy_h',
      'sale_of_stock_fy_h',
      'purchase_of_stock_fy_h',
      'issuance_of_debt_net_fy_h',
      'issuance_of_long_term_debt_fy_h',
      'reduction_of_long_term_debt_fy_h',
      'other_financing_cash_flow_items_total_fy_h',
      
      // Current values (for fallback)
      'cash_f_operating_activities_fy',
      'cash_f_investing_activities_fy',
      'cash_f_financing_activities_fy',
      'net_income_fy',
      'cash_flow_deprecation_n_amortization_fy',
      'funds_f_operations_fy',
      'changes_in_working_capital_fy',
      'capital_expenditures_fy',
      'capital_expenditures_fixed_assets_fy',
      'purchase_sale_investments_fy',
      'purchase_of_investments_fy',
      'sales_of_investments_fy',
      'purchase_of_business_fy',
      'sales_of_business_fy',
      'total_cash_dividends_paid_fy',
      'common_dividends_cash_flow_fy',
      'issuance_of_stock_net_fy',
      'sale_of_stock_fy',
      'purchase_of_stock_fy',
      'issuance_of_debt_net_fy',
      'issuance_of_long_term_debt_fy',
      'reduction_of_long_term_debt_fy',
      'other_investing_cash_flow_items_total_fy',
      'other_financing_cash_flow_items_total_fy',
      
      // Metadata
      'sector',
      'industry', 
      'sector-i18n-en',
      'industry-i18n-en',
      'report_type'
    ];
    
    // Build query parameters
    const queryParams = cashFlowFields.map(field => `fields=${field}`).join('&');
    
    // Fetch data from InsightSentry v3 API (same as balance sheet)
    const response = await fetch(
      `https://${RAPIDAPI_HOST}/v3/symbols/${encodeURIComponent(fullSymbol)}/fundamentals?${queryParams}`,
      {
        method: 'GET',
        headers: {
          'X-RapidAPI-Host': RAPIDAPI_HOST,
          'X-RapidAPI-Key': RAPIDAPI_KEY,
          'Accept': 'application/json'
        }
      }
    );

    if (!response.ok) {
      console.error(`API response not OK: ${response.status} ${response.statusText}`);
      throw new Error(`Failed to fetch data: ${response.status}`);
    }

    const responseData = await response.json();
    console.log(`API response received for ${fullSymbol}`);
    
    // Use ApiResponseParser to handle the response
    const parser = new ApiResponseParser();
    
    // Check if response has nested structure with numbered indices
    if (responseData && responseData.data) {
      // Check if data is an object with numbered keys (like the quarterly API)
      const dataKeys = Object.keys(responseData.data);
      const hasNumberedKeys = dataKeys.length > 0 && dataKeys.every(k => !isNaN(Number(k)));
      
      if (hasNumberedKeys) {
        // Convert numbered object to array for parser
        const dataArray = dataKeys.sort((a, b) => Number(a) - Number(b)).map(key => responseData.data[key]);
        console.log(`Converting numbered object to array: ${dataArray.length} items`);
        
        // Parse the array using ApiResponseParser
        const result = parser.parseApiResponse({ data: dataArray, metadata: responseData.metadata });
        
        if (result.errors.length > 0) {
          console.warn('Parser errors:', result.errors);
        }
        
        console.log('Parsed cash flow fields:', 
          Object.keys(result.data).filter(k => k.includes('cash_f_') || k.includes('_fy')).slice(0, 10)
        );
        
        return result.data;
      } else if (typeof responseData.data === 'object') {
        // Direct object format
        console.log('Direct object format, sample keys:', Object.keys(responseData.data).slice(0, 10));
        return responseData.data;
      }
    }
    
    console.log('No valid data structure found');
    return {};
    
  } catch (error) {
    console.error('Error fetching real API data:', error);
    // Return empty object to trigger fallback mock data
    return {};
  }
}

// Load data from local JSON file as fallback
function loadLocalApiData(symbol: string): any[] {
  try {
    const filePath = path.join(process.cwd(), 'api-response.json');
    if (!fs.existsSync(filePath)) {
      console.log('Local JSON file not found');
      return [];
    }
    
    const fileContent = fs.readFileSync(filePath, 'utf8');
    
    // Parse multiple JSON objects from file
    const jsonObjects: any[] = [];
    const lines = fileContent.split('\n');
    let currentJsonString = '';
    let braceCount = 0;
    
    for (const line of lines) {
      currentJsonString += line + '\n';
      
      // Count braces (simple approach)
      for (const char of line) {
        if (char === '{') braceCount++;
        if (char === '}') braceCount--;
      }
      
      // Complete JSON object found
      if (braceCount === 0 && currentJsonString.trim()) {
        try {
          const jsonObj = JSON.parse(currentJsonString);
          jsonObjects.push(jsonObj);
          currentJsonString = '';
        } catch (e) {
          // Continue if parse fails
        }
      }
    }
    
    // Find matching data for symbol
    const cleanSymbol = symbol.includes(':') ? symbol.split(':')[1] : symbol;
    
    for (const obj of jsonObjects) {
      if (obj.code) {
        const objSymbol = obj.code.includes(':') ? obj.code.split(':')[1] : obj.code;
        if (objSymbol.toUpperCase() === cleanSymbol.toUpperCase()) {
          console.log(`Found local data for ${symbol}`);
          return obj.data || [];
        }
      }
    }
    
    // Return first available data as fallback
    if (jsonObjects.length > 0 && jsonObjects[0].data) {
      console.log(`Using fallback local data from ${jsonObjects[0].code}`);
      return jsonObjects[0].data;
    }
    
    return [];
  } catch (error) {
    console.error('Error loading local data:', error);
    return [];
  }
}

export async function GET(
  request: NextRequest,
  { params }: { params: { symbol: string } }
) {
  try {
    const { symbol } = await params;
    
    if (!symbol) {
      return NextResponse.json(
        { error: 'Symbol parameter is required' },
        { status: 400 }
      );
    }

    console.log(`Fetching cash flow data for symbol: ${symbol}`);

    // Try to fetch real API data from RapidAPI
    let apiData = await fetchRealApiData(symbol.toUpperCase());
    let isFromLocalFile = false;
    
    // If API fails, try local JSON file
    if (!apiData || Object.keys(apiData).length === 0) {
      console.log('API failed, trying local JSON file');
      apiData = loadLocalApiData(symbol.toUpperCase());
      isFromLocalFile = true;
      
      if (!apiData || (Array.isArray(apiData) && apiData.length === 0)) {
        console.log('No data available, using mock fallback');
        const mockData = generateMockCashFlowData(symbol.toUpperCase());
        return NextResponse.json(mockData);
      }
    }

    // Map the API data to cash flow format
    const cashFlowData = mapApiToCashFlowData(apiData, symbol.toUpperCase(), isFromLocalFile);

    console.log('Processed cash flow data for', symbol, ':', {
      companyName: cashFlowData.company_name,
      sector: cashFlowData.sector,
      reportType: cashFlowData.report_type,
      hasOperatingCF: !!cashFlowData.cash_f_operating_activities_fy_h,
      hasInvestingCF: !!cashFlowData.cash_f_investing_activities_fy_h,
      hasFinancingCF: !!cashFlowData.cash_f_financing_activities_fy_h,
      fieldsAvailable: Object.keys(cashFlowData).filter(k => k.endsWith('_fy_h')).length
    });

    return NextResponse.json(cashFlowData);

  } catch (error) {
    console.error('Error in cash flow API:', error);
    return NextResponse.json(
      { 
        error: 'Internal server error',
        details: error instanceof Error ? error.message : 'Unknown error'
      },
      { status: 500 }
    );
  }
}