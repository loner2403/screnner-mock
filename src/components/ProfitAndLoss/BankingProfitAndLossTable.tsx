...
const BankingProfitAndLossTable: React.FC<Props> = ({ data }) => {
  const getCellClassName = (value: string | number | null, type: string, isNegative?: boolean, isSummary?: boolean) => {
    ...
    // Highlight summary rows (Total Revenue, Net Profit, etc.)
    if (isSummary) {
    ...
    <div className="w-full">
      ...
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-600">
          Consolidated Figures in Rs. Crores • Banking
        </p>
      </div>
        ...
        <table className="w-full table-auto min-w-full">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {data.rows.map((row, rowIndex) => {
              const isSummary = row.label.includes('Total') || row.label.includes('Profit') || row.label.includes('Revenue');
      ...
      <div className="mt-3 space-y-1">
        ...
        <p className="text-xs text-gray-500">
          * Banking data includes interest income, expenses, and regulatory metrics | + indicates expandable details
        </p>
        <p className="text-xs text-gray-500">
...
export default BankingProfitAndLossTable;
